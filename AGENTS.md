...
不得把模板动作当作保证命中；实际兵器轨迹由物理解算。不要声明尚未完成的盾牌、跨机器确定性、Mac 实时帧率或独立 AI 设计验收。开发样例与模型作品区分。自动测试、实际完整对局、网页可用性、动作观感分别记录。
...
2026-09-13用户明确：只做桌面观赛与录像，不做手机/触控适配或手机验收。重点是可读、有用的剑招和攻防，不以装修或测试数量替代整场观感。新增基础动作必须贯通配置、执行、判定、反馈和回放，需真实轨迹与防守/距离对照。
